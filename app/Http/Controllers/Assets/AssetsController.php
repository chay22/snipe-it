...
class AssetsController extends Controller
{
    ...
    public function store(ImageUploadRequest $request)
    {
        ...
        $_asset                    = new Asset();
        $_asset->name              = $request->input('name');
        $_asset->company_id        = Company::getIdForCurrentUser($request->input('company_id'));
        $_asset->model_id          = $asset_model ? $asset_model->id : null;
        $_asset->order_number      = $request->input('order_number');
        $_asset->notes             = $request->input('notes');
        $_asset->user_id           = Auth::id();
        $_asset->archived          = '0';
        $_asset->physical          = '1';
        $_asset->depreciate        = '0';
        $_asset->status_id         = $request->input('status_id', 0);
        $_asset->warranty_months   = $request->input('warranty_months');
        $_asset->purchase_cost     = Helper::ParseFloat($request->input('purchase_cost'));
        $_asset->purchase_date     = $request->input('purchase_date');
        $_asset->assigned_to       = $request->input('assigned_to');
        $_asset->supplier_id       = $request->input('supplier_id', 0);
        $_asset->requestable       = $request->input('requestable', 0);
        $_asset->rtd_location_id   = $request->input('rtd_location_id');

        if (!empty($settings->audit_interval)) {
            $_asset->next_audit_date   = Carbon::now()->addMonths($settings->audit_interval)->toDateString();
        }

        if ($_asset->assigned_to=='') {
        ...
        if ($request->input('assigned_user')) {
            ...
            $target = Location::find($request->input('assigned_location'));
            $location = $target->id;
        }
        ...
        foreach ($asset_tags as $i => $asset_tag) {
            ...
            if ($asset->isInvalid()) {
                return redirect()->back()->with('error_index', $i)->withInput()->withErrors($asset->getErrors());
            }
